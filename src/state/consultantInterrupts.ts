/**
 * Dr McGrath bedside interrupts (M39). She speaks during shifts when
 * specific safety-critical or rapport-collapse beats fire. Each
 * interrupt is composed deterministically from case-context so the
 * line lands as a presence in the room, not a generic alert.
 *
 ...
 * cs.consultantInterruptsFired). The composer never duplicates lines
 * across cases — every line cites the case name.
 */
...
export function composeTrapCaughtInterrupt(ctx: InterruptContext): ComposedInterrupt {
  ...
  return {
    line: `Hold on — ${action}? Walk me through your thinking. What's the cue you're seeing for ${ctx.caseName.toLowerCase()}?`,
    aside:
...
export function composeDeteriorationTakeoverInterrupt(ctx: InterruptContext): ComposedInterrupt {
  ...
  return {
    line: `${ctx.caseName} is ${verb}. I've got this — you watch. ${missingLine}`,
    aside: "(Stay in the room. We'll debrief this one properly afterwards.)",
  };
}

export function composeUnsafeMidshiftInterrupt(ctx: InterruptContext): ComposedInterrupt {
  return {
    line: `Step out with me a sec. ${ctx.caseName} hasn't gone the way we'd want. Take a breath — I'll cover for five.`,
