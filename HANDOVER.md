...
**Last session ended**: 2026-05-21, post-M97 closed (case-content deteriorating-vitals audit, last of the four BT 16/17 deferrals). M94 (vitals-strip layout repair) + M95 (kernel selectors + Monitor memoization) + M96 (clinical-cue sprite recipes for Marcus + asset library extension) + M97 (Williams stroke deteriorating vitals + AMBER+ test bar) all shipped. M93 + M92 + BT 17 + BT 16 + BT 15 synthesis complete.
...
### BT 16 + BT 17 deferred items — all shipped (M94/M95/M96/M97)

All four BT-deferred items closed in a follow-up sweep after M93's three-round review:

- ~~**M94 — vitals-strip layout repair**~~ **SHIPPED**. The `VitalsStripFrame` now declares `container-type: inline-size` and the value cells use `clamp(11px, 3.5cqi, 22px)` font sizing. At a 320-CSS-px patient panel each cell now sizes its content via the strip's own width via container queries; "132/82" and "36.6" render in full distinct columns instead of running together. Also added `flex: 1 1 0`, `min-width: 0`, real CSS `gap: 6px`, `tabular-nums`, and per-cell `text-overflow: ellipsis` as a belt-and-braces.
- ~~**M95 — kernel selectors + Monitor SVG memoization**~~ **SHIPPED**. New `useKernelSelector(selector)` hook in `src/state/sim.ts` wraps `useSim` + `useShallow` so consumers can subscribe to narrow kernel slices and only re-render when the projected value changes (instead of every tick). PatientPanel migrated as proof — reads `clockMin` via the selector. Monitor's two heavy SVG layers (bezel + CRT effects) extracted into `MonitorBezel` and `MonitorCRTEffects` memoized components — they render once and never re-render on tick changes. Pattern is documented for incremental migration of the other `useSim((s) => s.tick)` callsites (ShiftHubScreen / ShiftBoardScreen / ShiftView / EpisodeDebriefScreen / EncounterScreen).
- ~~**M96 — clinical-cue sprite recipes + asset library extension**~~ **SHIPPED**. The Asset Library at `?style-guide=1` previously only showed hand-authored `PATIENT_SPRITES` cases (5 cases); now also iterates `CASE_TO_DROP_ID` (16 total cases visible — finally surfaces the drop-mapped sprites for QA). Gallery capture revealed Marcus DKA was the actual content gap among the four BT 15 named cases: Leo paeds DKA had drowsy/Kussmaul, Joan head injury had forehead bruising, Ruby paeds anaphylaxis had cyanotic-lips + throat swelling, Beth adult anaphylaxis had swollen lips + sweat droplet + urticaria — but Marcus's deteriorating recipe was just pallor + unrelated leg marks. Added Leo's drowsy-eyes + open-Kussmaul-mouth pattern to Marcus's deteriorating recipe.
- ~~**M97 — case-content deteriorating-vitals audit**~~ **SHIPPED**. Computed NEWS2 for every case's `deteriorating` state block. 16/17 cases already produced NEWS2 ≥ 6 (AMBER/RED). The single outlier was `case_stroke_acute_williams` at NEWS2 = 4 (GREEN) — a deteriorating acute stroke landing in green band contradicts the clinical picture (haemorrhagic conversion / aspiration / raised ICP). Adjusted to NEWS2 = 8 (RED) by bumping RR to 22 (aspiration / Cheyne-Stokes), SpO2 to 93 (early aspiration), GCS to 12 (drowsier). The existing escalation test in `tests/vitals.test.ts` was tightened from `total ≥ 3 OR maxParam ≥ 3` to `total ≥ 5` so a future YAML edit can't silently re-introduce a sub-AMBER deteriorating state.

The BT 17 R3 reviewer's "deteriorating DKA shows NEWS2=0" was a screenshot-mislabelling artefact (the capture script's `getByRole(..., name: /marcus|dka/i).first()` regex fell through to `nth(1)` on a card-list where Marcus wasn't reachable by that name, so the captured patient was someone else's baseline-vitals card). The audit confirms Marcus's deteriorating NEWS2 is in fact 10 RED.

### Standing future work (no longer urgent, no review pressure)

- **Incremental kernel-selector migration** for the remaining tick-polling callsites (ShiftHubScreen / ShiftBoardScreen / ShiftView / EpisodeDebriefScreen / EncounterScreen). M95 added the hook + did PatientPanel; each subsequent migration is a small, easily-reviewed diff.
- **Semantic clinical-sign data-attributes** on sprite SVG output so tests can assert `data-clinical-sign="cyanosis"`-style presence instead of byte inequality. BT 17 approve_with_note re-reported across all three rounds. Needs an engine-shape change in the sprite renderer.
- **Capture-script reliability**: BT 17 R3 was misled by a capture script that named the wrong patient in its output filenames. Future review-bundle captures should verify the captured DOM matches the intended case (e.g., by reading the case id from a `data-case-id` attribute on PatientPanel before screenshotting).
